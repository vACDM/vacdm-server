...
export async function getAllPilots(filter: { [key: string]: any } = {}) {
  try {
    ...
    console.log('error getting all pilots', e);
    throw e;
  }
}
...
export async function getPilot(callsign: string): Promise<PilotDocument> {
  try {
    ...
    console.log('error getting pilot', callsign, e);
    throw e;
  }
}
...
export async function addLog(logData: Partial<PilotLog>) {
  ...
  try {
    ...
    console.log('error adding log', logData, e);
    throw e;
  }
...
async function calculations(pilot: PilotDocument): Promise<PilotDocument> {
  ...
  if (!pilot.inactive && timeUtils.isTimeEmpty(pilot.vacdm.tsat)) {
    try {
      const runwayDesignator: string = await airportService.determineRunway(
        pilot,
      );
  ...
  if (!pilot.inactive && !pilot.vacdm.manual_exot) {
    try {
      const exotBefore = Number(pilot.vacdm.exot);
      const taxizoneBefore = String(pilot.vacdm.taxizone);

      const taxizone = await airportService.determineTaxizone(pilot);
...
export async function addPilot(pilot: Pilot): Promise<PilotDocument> {
  ...
  try {
    ...
    console.log('error adding pilot', e);
    throw e;
  }
...
export async function deletePilot(callsign: string): Promise<void> {
  try {
    ...
    console.log('error deleting pilot', callsign, e);
    throw e;
  }
}
...
export async function doesPilotExist(callsign: string): Promise<boolean> {
  try {
    ...
    console.log('error checking if pilot exists', e);
    throw e;
  }
}
...
export async function updatePilot(
  ...
  try {
    ...
    // TODO: super weird defining changes this way as it otherwise breaks line below
    const changesOps: Partial<Pilot> = nestedobjectsUtils.getValidUpdateOpsFromNestedObject(changes);
    ...
    console.log('error updating pilot', e);
    throw e;
  }
}
...
export async function getPilotLogs(
  ...
  try {
    ...
    console.log('error getting logs for pilot', callsign, e);
    throw e;
  }
}
